        ...
        <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden;">
            <div style="padding: 20px; text-align: center; border-bottom: 1px solid #e2e8f0;">
                <img src="https://res.cloudinary.com/daeczbv7v/image/upload/v1758805746/Metropolitan_logo_semfundo_as8xh0.png"
                     alt="Metropolitan Logo" style="width: 140px; height: auto;"/>
            </div>
            <div style="padding: 30px; line-height: 1.6; color: #333;">
                <h2 style="font-size: 20px; color: #1a202c; margin-top: 0;">Seu Novo Código de Verificação</h2>
                <p style="font-size: 16px;">Olá <strong>{user.full_name}</strong>,</p>  <!-- ✅ user, not novo_usuario -->
                <p style="font-size: 16px;">Utilize o código abaixo para ativar sua conta:</p>
                <div style="background-color: #f7fafc; padding: 20px; text-align: center; border-radius: 6px; margin: 25px 0;">
                    <p style="font-size: 32px; letter-spacing: 8px; margin: 0; color: #2d3748; font-weight: bold;">{new_code}</p>  <!-- ✅ new_code, not verification_code -->
                </div>
                <p style="font-size: 14px; color: #555;">Este código expira em <strong>15 minutos</strong>.</p>
            </div>
...
def login_for_access_token(
    ...
    x_2fa_session_token: Optional[str] = Header(default=None, alias="X-2FA-Session-Token"),
):
    ...
    if user.totp_enabled:
        if x_2fa_session_token:
            try:
                _exc = HTTPException(status_code=401, detail="")
                email = security.verify_token(x_2fa_session_token, _exc, expected_type="2fa_session")
                if email == user.email:
...
def request_password_reset(
    ...
    if user:
        ...
        reset_link = f"https://metropolitaninvestimentos.com/redefinir-senha?token={reset_token}"
        html_body = f"""
        <div style="font-family: Arial, sans-serif; background-color: #f4f4f7; padding: 20px;">
    <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden;">
...
@router.post("/forgot-password-admin", status_code=status.HTTP_200_OK)
def request_password_reset_admin(
    ...
    if user:
        ...
        reset_link = f"https://painel.bancometropolitan.com.br/redefinir-senha?token={reset_token}"
